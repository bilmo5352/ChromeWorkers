...
// Process a single URL with retry logic and anti-detection measures
async function processUrl(browser, targetUrl, urlIndex) {
    let context = null;
    let page = null;
    let success = false;
    let lastError = null;
    ...
    for (let attempt = 0; attempt < maxRetries && !success; attempt++) {
        try {
            ...
            if (attempt < PROXY_SERVERS.length) {
                ...
                const startIndex = urlIndex % PROXY_SERVERS.length;
    ...
    // If all attempts failed, clean up and return error
    if (page) {
        try { await page.close(); } catch (e) {}
    }
    if (context) {
...
app.post('/render', async (req, res) => {
    ...
    try {
        ...
        console.log(`Processing ${targetUrls.length} URLs in parallel...`);
        const results = await Promise.all(
            targetUrls.map((targetUrl, index) => processUrl(browser, targetUrl, index))
        );
        res.json({ results });
